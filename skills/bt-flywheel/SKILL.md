...
| **Datasets** | Braintrust | `bt datasets` CLI or Python SDK |
...
**Step 5 — Facet distribution** (if facets discovered): GROUP BY facet column to understand input topic coverage and spot underrepresented categories.
...
Use `bt datasets list/get` if available. If `bt datasets` is not available (check with `bt --help`), use `bt sql` to inspect existing dataset content and the Python SDK for writes.

### Updating Scorers

Load `references/bt-functions-patterns.md`. Note: `bt functions` may not be available in all `bt` versions — check with `bt --help` first. If unavailable, use the Braintrust UI to read and update scorers.
...
4. Autonomous mode completes one full loop iteration, routes to "done", and writes the summary file
